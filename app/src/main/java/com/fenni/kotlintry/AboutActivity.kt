...
import com.fenni.kotlintry.Security.verifyPurchase
...
class AboutActivity : AppCompatActivity(), PurchasesUpdatedListener {
    ...
    private val preferenceObject: android.content.SharedPreferences?
        get() = applicationContext.getSharedPreferences(PREF_FILE, 0)

    private val preferenceEditObject: android.content.SharedPreferences.Editor
        get() {
    ...
    private val purchaseValueFromPref: Boolean
        get() = preferenceObject?.getBoolean(PURCHASE_KEY, false) == true
    ...
    companion object {
        const val PREF_FILE = "Donation"
        const val PURCHASE_KEY = "donation_lovetime"
        const val PRODUCT_ID = "donation_lovetime"
    }
}
